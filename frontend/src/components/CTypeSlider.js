...
const CTypeSlider = ({ viewAll }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://54.169.210.228:5000/api/course-category/categories');
        ...
        await Promise.all(fetchedCategories.map(async (category) => {
          try {
            const feeResponse = await axios.get(`http://54.169.210.228:5000/api/courses/lowest-fee/${category.name}`);
